...
const LoginPage = () => {
  ...
  const [formObj, setFormObj] = useState({
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      const cred = await signInWithEmailAndPassword(auth, formObj.email, formObj.password);
        ...
        <div className='w-[40%] max-w-[500px] min-w-[250px] mx-auto p-8 bg-black/90 backdrop-blur'>
            <div className='flex flex-col space-y-5'>
                ...
                <form className='flex flex-col space-y-5' onSubmit={(e) => handleSubmit(e)}>
                    ...
                    <button className='bg-red p-3 text-[1.3rem] rounded' type="submit">Sign In</button>
                </form>
                <div className='flex items-center justify-between'>
                  <p className='text-lightGray'>Already have an account?</p>
                  <Link to="/signup"> <p className='text-lightGray hover:text-white'>Sign Up</p></Link>
                </div>
            </div>
        </div>
    </div>
